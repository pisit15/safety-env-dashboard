...
export async function GET(request: NextRequest) {
  ...
  const planType = searchParams.get('planType') as 'safety' | 'environment';
  ...
  try {
    ...
    const ws = workbook.addWorksheet(`${company.shortName} ${planType === 'safety' ? 'Safety' : 'Environment'} Plan`);
    ...
    const filename = `${company.shortName}_${planType}_plan_${new Date().toISOString().slice(0, 10)}.xlsx`;
