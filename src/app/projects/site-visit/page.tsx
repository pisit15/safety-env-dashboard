...
export default function SiteVisitOverview() {
  ...
  const companyIds = auth.isAdmin
    ? COMPANIES.map(c => c.id)
    : Object.keys(auth.companyAuth);
  ...
  async function fetchAssessments() {
    ...
    try {
      ...
      if (json.data) {
        const filtered = auth.isAdmin
          ? json.data
          : json.data.filter((a: Assessment) => companyIds.includes(a.company_id));
  ...
  return (
    <div style={{ padding: '32px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
        ...
        {companyIds.length === 1 && (
          <Link
            href={`/projects/site-visit/${companyIds[0]}/assess`}
            style={{
