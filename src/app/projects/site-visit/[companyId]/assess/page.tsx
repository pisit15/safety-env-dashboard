...
export default function AssessmentForm() {
  ...
  const [assessmentType, setAssessmentType] = useState<'self' | 'admin'>(auth.isAdmin ? 'admin' : 'self');
    ...
    <div style={{ padding: '24px 32px', maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, color: VIZ.text, display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          {existingId ? 'แก้ไขการประเมิน' : 'ประเมินใหม่'} — {company?.name || companyId}
        </h1>
      </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
          <div>
            ...
            >
              ...
              {auth.isAdmin && <option value="admin">Admin Audit</option>}
            </select>
          </div>
          <div>
